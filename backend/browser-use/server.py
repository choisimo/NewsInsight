...
async def request_human_intervention(
    ...
    try:
        await asyncio.wait_for(
            job.intervention.response_event.wait(),
            timeout=job.intervention.timeout_seconds
        )
...
async def run_browser_task(job: Job):
    ...
    try:
        ...
        llm = ChatAIDove(
            session_id=job.session_id or f"browser-{job.id}",
            timeout=120.0,
            max_retries=3
        )
        ...
        profile = BrowserProfile(
            headless=True if is_docker else job.headless,  # Force headless in Docker
        ...
        # Build task with optional URL
        full_task = job.task
        if job.url:
            full_task = f"First navigate to {job.url}, then: {job.task}"
...
State the problem clearly so a human operator can assist."""
        ...
        try:
            result = await asyncio.wait_for(
                agent.run(max_steps=job.max_steps),
            ...
            job.result = str(result) if result else "Task completed successfully"
        ...
        while True:
            ...
            if data.get("type") == "intervention_response":
                ...
                await websocket.send_json({
                    ...
                    "message": "Action received and will be executed"
                })
                ...
                action = await request_human_intervention(
                    job,
                    intervention_type,
                    reason,
...
async def submit_intervention(job_id: str, action: HumanAction):
    ...
    if job.status != JobStatus.WAITING_HUMAN:
        raise HTTPException(
            status_code=400, 
            detail=f"Job is not waiting for intervention. Current status: {job.status.value}"
        )
...
async def manual_intervention_request(
    ...
    if job.status != JobStatus.RUNNING:
        raise HTTPException(
            status_code=400,
            detail=f"Can only request intervention for running jobs. Current status: {job.status.value}"
        )
