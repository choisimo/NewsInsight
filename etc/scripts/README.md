...
1. Reads configuration from `etc/configs/{environment}.env`
...
```
- Ensure the config file exists in the `etc/configs/` directory
...
This script is automatically run by the `consul-seed` init container in `etc/docker/docker-compose.consul.yml`:
...
For more information, see [etc/infra-guides/CONSUL_MIGRATION.md](../infra-guides/CONSUL_MIGRATION.md)
